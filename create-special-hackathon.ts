...
async function createSpecialHackathon() {
  try {
    ...
    // Les dates demandées pour le 22 Avril 2026
    // On utilise le fuseau horaire local (GMT+1 pour la Tunisie/France en hiver, ou ajusté selon le système)
    // Le format ISO avec offset assure la précision.
    ...
    const competition = await prisma.competition.create({
      data: {
        ...
        difficulty: "MEDIUM",
        specialty: null, // Toutes catégories (null = GENERAL)
        startDate: startDate,
        endDate: endDate,
        status: "RUNNING", // On le met en RUNNING car 21:40 est déjà passé
        isActive: true,
