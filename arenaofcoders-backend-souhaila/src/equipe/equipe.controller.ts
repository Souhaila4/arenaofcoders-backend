...
export class EquipeController {
  ...
  @ApiQuery({ name: 'query', description: 'Search by name or email', required: false })
  ...
  async searchUsers(
    ...
    return this.equipeService.searchUsers(query, competitionId, specialty);
  }
  ...
  @Get('equipes/:id/synergy')
  @ApiOperation({
    summary: 'AI Team Synergy Predictor — analyse team skill balance',
    description:
      'Analyses team members\' specialties, skill tags, and GitHub repos to compute a radar chart of strengths/weaknesses across 6 axes (Frontend, Backend, AI/Data, Mobile, DevOps, Design). Returns a synergy score and strategic French advice.',
  })
  @ApiParam({ name: 'id', description: 'Equipe ID' })
  @ApiResponse({ status: 200, description: 'Synergy analysis with radar data and advice' })
  @ApiResponse({ status: 404, description: 'Equipe not found' })
  async getTeamSynergy(@Param('id') equipeId: string) {
  ...
  @Delete('equipes/:id/members/:userId')
  @UseGuards(RolesGuard)
  @Roles(UserRole.USER)
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Remove a member from the equipe (leader only)' })
  @ApiParam({ name: 'id', description: 'Equipe ID' })
  @ApiParam({ name: 'userId', description: 'User ID of the member to remove' })
  @ApiResponse({ status: 200, description: 'Member removed successfully' })
  @ApiResponse({ status: 403, description: 'Not the team leader' })
  @ApiResponse({ status: 404, description: 'Equipe or Member not found' })
  async removeMember(
    @Param('id') equipeId: string,
