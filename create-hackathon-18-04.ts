...
async function createHackathon() {
  try {
    ...
    // Hackathon: 10:10 to 11:30
    // Checkpoint 1: 10:10 to 10:25
    
    // Note: Prisma/MongoDB usually stores dates in UTC. 
    // If the local time is +01:00, shifted accordingly.
    // However, for consistency with existing scripts, we'll use UTC dates that match the local time intent.
    ...
    const competition = await prisma.competition.create({
      data: {
        ...
        specialty: null, // "ne pas faire specialite" -> null means all can join
        startDate: startDate,
        endDate: endDate,
