...
async function createTestHackathon() {
    try {
        ...
        startDate.setHours(11, 10, 0, 0);
        const endDate = new Date(today);
        endDate.setHours(11, 30, 0, 0);
        const cp1Date = new Date(today);
        cp1Date.setHours(11, 15, 0, 0);
        const cp2Date = new Date(today);
        cp2Date.setHours(11, 20, 0, 0);
        const cp3Date = new Date(today);
        cp3Date.setHours(11, 25, 0, 0);
        console.log('Creating hackathon...');
        const competition = await prisma.competition.create({
            data: {
                title: "Hackathon Rapide 11:10",
                description: "Hackathon de test avec 3 checkpoints rapides.",
                difficulty: "EASY",
                specialty: "FULLSTACK",
                startDate: startDate,
                endDate: endDate,
...
createTestHackathon();
//# sourceMappingURL=create-hackathon-11-10.js.map