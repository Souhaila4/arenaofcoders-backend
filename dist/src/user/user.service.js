...
let UserService = class UserService {
    ...
    async requestCompanyRole(userId, dto) {
        ...
        if (existing) {
            throw new common_1.ConflictException('You already have a pending company role request.');
        }
    ...
    async getLeaderboard() {
        ...
        const usersWithXP = users.map((user) => {
            const daysSinceCreation = Math.floor((Date.now() - new Date(user.createdAt).getTime()) /
                (1000 * 60 * 60 * 24));
