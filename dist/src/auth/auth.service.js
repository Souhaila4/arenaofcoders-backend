...
let AuthService = class AuthService {
    ...
    streamService;
    jwtExpiresIn;
    constructor(userService, jwtService, config, cvExtractionService, apifyService, scraperService, passwordResetService, streamService) {
        ...
        this.jwtExpiresIn = this.config.get('JWT_EXPIRES_IN', '7d');
    }
    async signUp(dto, resumeBuffer, avatarFile) {
        try {
            let avatarUrl = null;
            ...
            void this.streamService
                .ensureRoomMember(user.id, room_specialty_config_1.GENERAL_MEMBER_ROOM_ID)
                .catch((err) => {
                const msg = err instanceof Error ? err.message : String(err);
                console.warn(`[SIGNUP] Could not add user to general member room (${room_specialty_config_1.GENERAL_MEMBER_ROOM_ID}): ${msg}`);
            });
            const payload = {
                sub: user.id,
                email: user.email,
    ...
    async uploadAvatarAndUpdateProfile(userId, avatarFile) {
        const uploadsDir = path.join(process.cwd(), 'uploads', 'avatars');
