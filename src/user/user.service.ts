...
export class UserService {
  ...
  async requestCompanyRole(userId: string, dto: { companyName: string; description?: string }) {
    const existing = await this.prisma.companyRoleRequest.findFirst({
      where: { userId, status: 'PENDING' },
    });
    if (existing) {
      throw new ConflictException('You already have a pending company role request.');
    }
  ...
  async create(data: CreateUserInput) {
    ...
    return this.prisma.user.create({
      data: {
        ...
        ...(data.avatarUrl != null && data.avatarUrl !== '' && { avatarUrl: data.avatarUrl }),
