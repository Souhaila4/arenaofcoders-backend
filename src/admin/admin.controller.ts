...
export class AdminController {
  ...
  @Get('recipients/by-hedera')
  @ApiOperation({
    summary: 'Résoudre un utilisateur par son Hedera account ID (avant envoi ARENA)',
    description:
      'Utilisé par l’app mobile admin pour afficher le destinataire avant mint.',
  })
  @ApiQuery({
    name: 'hederaAccountId',
    required: true,
  ...
  @ApiResponse({ status: 200 })
  async getWalletRecipientByHedera(
    @Query('hederaAccountId') hederaAccountId: string,
  ) {
    return this.walletService.getWalletRecipientPreviewForAdmin(
      hederaAccountId ?? '',
    );
  }
