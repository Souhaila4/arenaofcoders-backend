...
export class AuthController {
  ...
  @UseInterceptors(
    FileFieldsInterceptor([
      ...
      fileFilter: (_req, file, cb) => {
        if (file.fieldname === 'resume' && !file?.originalname?.toLowerCase().endsWith('.docx')) {
          return cb(
  ...
  @ApiBody({
    schema: {
      ...
      properties: {
        ...
        avatar: { type: 'string', format: 'binary', description: 'Avatar Image for Facial Verification (required)' },
  ...
  @ApiResponse({ status: 201, description: 'User created, verification code sent to email. No tokens until email is verified.' })
  @ApiResponse({ status: 400, description: 'Validation error or missing/invalid resume/avatar' })
  @ApiResponse({ status: 409, description: 'Email already registered' })
  async signUp(
    @Body() dto: SignUpDto,
    @UploadedFiles() files: { resume?: Express.Multer.File[]; avatar?: Express.Multer.File[] },
  ) {
    ...
    if (!avatarFile?.buffer) {
      throw new BadRequestException('Avatar image file is required for identity verification');
    }
  ...
  @ApiResponse({ status: 200, description: 'Email verified, returns user and tokens' })
