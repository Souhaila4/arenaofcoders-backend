  ...
  async signUp(
    ...
    try {
      ...
      if (resumeBuffer?.length) {
        console.log(`[SIGNUP] 📄 Resume detected (${resumeBuffer.length} bytes). Starting CV extraction...`);
        try {
          const extraction =
            await this.cvExtractionService.extractFromBuffer(resumeBuffer);
          console.log(`[SIGNUP] ✅ CV extraction success:`, {
            mainSpecialty: extraction.mainSpecialty,
            skillTags: extraction.skillTags,
            rawPrediction: extraction.rawPrediction,
          ...
          if (Object.keys(updateDto).length > 0) {
            ...
            console.log(`[SIGNUP] ✅ Profile updated with CV data:`, updateDto);
          } else {
            console.log(`[SIGNUP] ⚠️ CV extraction returned no usable data (no specialty or skills)`);
          }
        } catch (err) {
          console.error(`[SIGNUP] ❌ CV extraction FAILED:`, err instanceof Error ? err.message : err);
          // CV extraction failed (e.g. Hugging Face timeout) — signup continues without it
        }
      } else {
        console.log(`[SIGNUP] ℹ️ No resume file provided, skipping CV extraction`);
      }
