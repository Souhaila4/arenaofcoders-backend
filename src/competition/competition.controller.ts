...
export class CompetitionController {
  ...
  @ApiResponse({ status: 404, description: 'Competition not found' })
  async archiveCompetition(@Param('id') competitionId: string, @CurrentUser() user: any): Promise<unknown> {
  ...
  async findAllCompetitions(@Query() queryDto: CompetitionQueryDto, @CurrentUser() user: any) {
  ...
  @ApiResponse({ status: 200, description: 'Top participants with scores and AI reports' })
  @ApiResponse({ status: 404, description: 'Competition not found' })
  ...
  @ApiParam({ name: 'id', description: 'MongoDB ObjectId of the competition' })
  @ApiResponse({ status: 200, description: 'All participants with detailed info' })
  ...
  @ApiParam({ name: 'id', description: 'MongoDB ObjectId of the competition' })
  @ApiParam({ name: 'participantId', description: 'MongoDB ObjectId of the participant to select as winner' })
    ...
    FileInterceptor('hackathonFaceImage', {
      ...
      fileFilter: (_req, file, cb) => {
        ...
        if (!isImageMime && !isImageExt) {
          return cb(new BadRequestException('Face Image must be an image file'), false);
        }
        cb(null, true);
      },
    }),
  ...
  @HttpCode(HttpStatus.CREATED)
  @ApiOperation({ summary: 'Join a competition and provide face image (Anti-Cheat)' })
  @ApiParam({ name: 'id', description: 'MongoDB ObjectId of the competition' })
  @ApiBody({
    schema: {
      type: 'object',
      ...
      properties: {
        hackathonFaceImage: { type: 'string', format: 'binary', description: 'Personal Image for Facial Verification' },
      },
    },
  })
  @ApiResponse({ status: 201, description: 'Successfully joined the competition' })
  @ApiResponse({ status: 400, description: 'Competition not OPEN / Missing image' })
  ...
  @ApiResponse({ status: 404, description: 'Competition not found' })
  async getCheckpointSubmissionsForReview(@Param('id') competitionId: string, @CurrentUser() user: any) {
