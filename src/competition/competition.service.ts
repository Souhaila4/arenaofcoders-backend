...
export class CompetitionService {
  ...
  async selectWinner(
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (oldWinnerId) {
        const oldParticipant = await tx.competitionParticipant.findUnique({
          ...
          include: { equipe: { include: { members: true } } },
        });

        if (oldParticipant) {
          let oldUserIds = [oldParticipant.userId];
          if (oldParticipant.equipeId && oldParticipant.equipe?.members) {
            oldUserIds = oldParticipant.equipe.members.map((m) => m.userId);
          }
    ...
    // 6. Release reward (Split: 60% / 30% / 10%)
    let rewardResult: any = null;
    if (competition.rewardPool > 0) {
      const split = [0.6, 0.3, 0.1];
      const percentage = split[rank - 1] || 0;
      const amountToRelease = competition.rewardPool * percentage;
