...
export class CompetitionService {
  ...
  async getCompetitionsWonByUser(userId: string) {
    const participations = await this.prisma.competitionParticipant.findMany({
      ...
      include: {
        competition: {
          include: {
            creator: {
              select: { firstName: true, lastName: true, id: true, email: true, avatarUrl: true },
            },
            _count: { select: { participants: true } },
          },
        },
    ...
    return {
      data: participations.map((p) => p.competition).map((comp: any) => this.censorPreHackathonDetails(comp)),
      pagination: {
        page: 1, limit: 100, totalCount: participations.length, totalPages: 1, hasNext: false, hasPrev: false,
      }
    };
  }
      ...
      for (const sub of overduePending) {
        ...
        const failedCount = await this.prisma.checkpointSubmission.count({
          where: {
            ...
            status: { in: [CheckpointStatus.MISSED, CheckpointStatus.REJECTED] },
          },
        });
