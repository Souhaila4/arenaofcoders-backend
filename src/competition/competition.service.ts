...
export class CompetitionService {
  ...
  /**
   * CRÉATION D'UN HACKATHON (Competition)
   * 
   * Cette méthode est appelée par un Admin (ou Entreprise) pour créer un nouveau hackathon.
   * Elle vérifie que la durée minimale est de 8h (nécessaire pour la génération des Checkpoints).
   * Elle sauvegarde le Hackathon, génère automatiquement les Checkpoints, notifie les utilisateurs,
   * et verrouille les fonds de récompense (Reward Pool) sur Hedera.
   */
  ...
  /**
   * GÉNÉRATION AUTOMATIQUE DES CHECKPOINTS
   * 
   * Appelée lors de la création d'un hackathon.
   * Calcule automatiquement le nombre de checkpoints en fonction de la durée totale.
   * - Hackathon < 19h -> Checkpoint toutes les 4h
   * - Hackathon >= 19h -> Checkpoint toutes les 6h
   * 
   * Les checkpoints s'ouvrent à intervalles réguliers et les participants ont
   * 15 minutes pour soumettre leur avancement (Anti-Cheat).
   */
  ...
  /**
   * SOUMISSION D'UN CHECKPOINT (Pilier Central)
   * 
   * Cette méthode gère la réception d'un checkpoint par un utilisateur ou une équipe.
   * Elle vérifie :
   * 1. Que le participant est bien inscrit et non banni.
   * 2. Que le checkpoint est soumis dans la fenêtre temporelle autorisée (ex: 15 min avant la deadline).
   * 3. Si l'utilisateur est en équipe, que seul le LEADER a le droit de valider au nom du groupe.
   * 4. Transfère la logique d'évaluation GitHub / IA vers `executeAdvancedCheckpointLogic`.
   */
  async submitCheckpoint(
    ...
    // Calcul de la fenêtre d'ouverture (ex: 15 minutes avant l'heure de fin)
    const opensAt = new Date(
    ...
    // Si le participant fait partie d'une équipe, seul le LEADER peut faire le submit.
    // L'avancement sera ensuite propagé à l'ensemble des membres (score IA, baseRepositoryUrl, extraLife).
    if (participation.equipeId) {
      const membership = await this.prisma.equipeMember.findFirst({
        where: { equipeId: participation.equipeId, userId },
      });
  ...
  /**
   * LOGIQUE AVANCÉE D'ÉVALUATION ET D'AVANCEMENT GITHUB (Anti-Cheat Temporel)
   * 
   * Cette méthode est le coeur du moteur d'évaluation des hackathons.
   * 
   * CP1 (Checkpoint 1) : Validation stricte de l'URL GitHub (Repository Locking).
   *                      Ce repo devient la "Base" obligatoire pour la suite.
   * 
   * CP>1 (Checkpoints Suivants) : 
   *  - Anti-Cheat : Vérifie que l'URL commence bien par la `baseRepositoryUrl`.
   *  - Évaluation : Utilise RepoExtractorAgent pour extraire l'arborescence et le code.
   *  - Objectif : Compare le nombre de fichiers (`fileCount`) au précédent checkpoint. 
   *               S'il n'y a pas d'évolution, le checkpoint est rejeté.
   *  - Subjectif : Appelle le CodeJudgeAgent pour analyser la qualité et donner des feedbacks (warnings/highlights).
   *  - Grace Period ("Joker") : Si le code est invalide, donne 1 chance supplémentaire avant la disqualification.
   */
