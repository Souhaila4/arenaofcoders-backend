...
export class CompetitionService {
  ...
  async createCompetition(
    ...
    if (durationMs < 3 * 60 * 60 * 1000) {
      throw new BadRequestException('La durée du hackathon doit être d\'au moins 3 heures.');
    }
    ...
    if ((createData.rewardPool ?? 0) > 0) {
      void this.walletService
        .lockEscrow(adminUserId, createData.rewardPool as number, competition.id)
        .catch((err) =>
          this.logger.warn(`Escrow lock skipped for competition ${competition.id}: ${err?.message ?? err}`),
        );
    }
  ...
  private async createDefaultCheckpoints(
    ...
    // Le 1er checkpoint débute 2 HEURES après le début du hackathon
    let currentCpOpensAt = new Date(startDate.getTime() + 2 * 60 * 60 * 1000);

    // Condition d'arrêt: Le checkpoint ne peut pas s'ouvrir si ça dépasse (Heure de Fin - 1 heure)
    const limitLimit = new Date(endDate.getTime() - 1 * 60 * 60 * 1000);

    while (currentCpOpensAt <= limitLimit) {
      // La durée de validation d'un checkpoint est de 15 minutes
      const currentCpDue = new Date(currentCpOpensAt.getTime() + 15 * 60 * 1000);

      defaults.push({
        competitionId,
        title: `Checkpoint ${cpIndex}`,
        description: `Évaluation Anti-Triche automatique par l'IA (Checkpoint ${cpIndex}). Vous avez 15 minutes à partir de l'ouverture pour soumettre.`,
        order: cpIndex,
        dueDate: currentCpDue, // Heure limite de validation de ce checkpoint
        isMandatory: true,
      });

      // Le système ouvre un checkpoint chaque 2 heures.
      currentCpOpensAt = new Date(currentCpOpensAt.getTime() + 2 * 60 * 60 * 1000);
      cpIndex++;
    }

    if (defaults.length === 0) {
      // Fallback au cas où le hackathon est très court (< 3h)
      defaults.push({
        competitionId,
  ...
  async getAllParticipantsForAdmin(competitionId: string) {
    ...
    const participants = await this.prisma.competitionParticipant.findMany({
      ...
      orderBy: [{ status: 'asc' }, { score: 'desc' }] as Prisma.CompetitionParticipantOrderByWithRelationInput[],
    });
  ...
  async selectWinner(
    ...
    if (competition.createdBy !== adminUserId) {
      ...
      if (!adminUser || adminUser.role !== UserRole.ADMIN) {
        throw new ForbiddenException('Only the hackathon creator or an admin can select a winner.');
      }
    }
    ...
    if (participant.status !== ParticipantStatus.SUBMITTED) {
      throw new BadRequestException('Only submitted participants can be selected as winner.');
    }
  ...
  async submitWork(competitionId: string, userId: string, githubUrl: string) {
    ...
    // La soumission finale ouvre uniquement 1 heure avant la fin du hackathon
    const now = new Date();
    const submissionOpensAt = new Date(competition.endDate.getTime() - 60 * 60 * 1000);
    
    if (now < submissionOpensAt) {
      const formattedTime = submissionOpensAt.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' });
      throw new BadRequestException(
        `La soumission finale (GitHub) ne s'ouvre que 1 heure avant la fin du hackathon. (Ouverture prévue à ${formattedTime})`
      );
    }
  ...
  private censorPreHackathonDetails(competition: any, userRole?: string, userId?: string) {
    if (!competition) return competition;
    if (competition.status === CompetitionStatus.SCHEDULED || competition.status === CompetitionStatus.OPEN_FOR_ENTRY) {
      if (userRole === UserRole.ADMIN || (userId && competition.createdBy === userId)) {
        return competition;
      }
      return {
        ...competition,
