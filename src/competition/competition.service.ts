...
export class CompetitionService {
  ...
  async getTopParticipants(competitionId: string, limitOverride?: number) {
    ...
    // 1. Fetch all leader memberships to identify team representatives
    const leaders = await this.prisma.equipeMember.findMany({
      where: {
        role: EquipeMemberRole.LEADER,
        equipe: { competitionId },
      },
      select: { userId: true, equipeId: true, equipe: { select: { name: true } } },
    });
    const leaderMap = new Map(leaders.map((l) => [l.userId, l]));

    // 2. Fetch all submitted participants sorted by score
    // We fetch a bit more than 'take' to ensure we have enough after filtering
    const allCandidates = await this.prisma.competitionParticipant.findMany({
      where: {
        competitionId,
      ...
      take: take * 4, // Multiplier to handle large teams
    });

    // 3. Filter: Keep only Solo participants or Team Leaders
    const filtered = allCandidates.filter((p) => {
      if (!p.equipeId) return true;
      return leaderMap.has(p.userId);
    }).slice(0, take); // Apply the limit after filtering

    return {
      competitionId,
      topN: take,
      winnerId: competition.winnerId ?? null,
      preselected: filtered.map((p, index) => {
        const teamInfo = p.equipeId ? leaderMap.get(p.userId) : null;
        return {
          rank: index + 1,
          participantId: p.id,
          ...
          user: p.user,
          equipeId: p.equipeId,
          equipeName: teamInfo?.equipe?.name,
        };
      }),
    };
  }
  ...
  async getAllParticipantsForAdmin(competitionId: string) {
    ...
    // 2. Fetch all leader memberships for this competition to identify team representatives
    const leaders = await this.prisma.equipeMember.findMany({
      where: {
        role: EquipeMemberRole.LEADER,
        equipe: { competitionId },
    ...
    const filtered = allParticipants.filter((p) => {
      if (!p.equipeId) return true; // Solo participant
      return leaderMap.has(p.userId); // Keep only if they are the leader
    });
  ...
  async selectWinner(
    ...
    if (participant.equipeId) {
      ...
      await this.prisma.competitionParticipant.updateMany({
        where: { equipeId: participant.equipeId, competitionId },
        data: { isWinner: true },
      });

      // Get all team members to update their User profiles
      const members = await this.prisma.equipeMember.findMany({
        where: { equipeId: participant.equipeId },
        select: { userId: true },
      });
      const memberUserIds = members.map((m) => m.userId);

      // Increment wins for everyone in the team
      ...
      // Create notifications for everyone in the team
      for (const uid of memberUserIds) {
        await (this.prisma as any).notification.create({
          data: {
            userId: uid,
            type: 'HACKATHON_WINNER',
            title: `🏆 Votre équipe a gagné le hackathon "${competition.title}" !`,
            body: `Félicitations ! Votre équipe a été sélectionnée comme gagnante.\n\nContact de l'organisateur:\nNom: ${creator?.firstName ?? ''} ${creator?.lastName ?? ''}\nEmail: ${creator?.email ?? 'Non disponible'}`,
            competitionId,
          },
        });
      }
    ...
    // Update competition winnerId (we store the representative/leader ID)
    ...
    this.logger.log(
      `Winner selected (Team/Solo): participant ${participantId} for competition ${competitionId}`,
    );
  ...
  async submitWork(competitionId: string, userId: string, githubUrl: string) {
    this.logger.log(`🔍 [submitWork] competitionId=${competitionId} userId=${userId} githubUrl=${githubUrl}`);
    const competition = await this.findCompetitionById(competitionId);
    ...
    if (participation.equipeId) {
      ...
      if (equipe?.submittedAt) {
        // Idempotent: if the team already submitted, return success instead of error
        // This handles double-click / race conditions gracefully
        return {
          status: 'SUBMITTED',
          message: 'Your team has already submitted work for this competition.',
          participation,
        };
      }
    }
    ...
    if (participation.status === ParticipantStatus.SUBMITTED) {
      ...
      return {
        status: 'SUBMITTED',
        message: 'You have already submitted your work for this competition.',
        participation,
      };
    }
    ...
    if (competition.antiCheatEnabled) {
      ...
      if (score > threshold) {
        ...
        if (dq.count === 0) {
          ...
          return {
            status: 'DISQUALIFIED',
            message: 'You have been disqualified from this competition and cannot submit again.',
            participation,
          };
        }
      ...
      if (equipeId) {
        ...
        if (ok.count === 0) {
          ...
          return {
            status: 'SUBMITTED',
            message: alreadySubmittedMessage,
            participation,
          };
        }
      }
  ...
  private async markEquipeSubmittedTx(
    ...
    if (eq.count === 0) {
      // Idempotent: another concurrent request already submitted for this team.
      // We can just return early, the outer function will return success.
      return;
    }
