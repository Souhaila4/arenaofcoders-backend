...
import { InterviewPracticeModule } from './interview-practice/interview-practice.module';
...
@Module({
  imports: [
    ConfigModule.forRoot({
      isGlobal: true,
      envFilePath: envFilePaths.length
        ? envFilePaths
        : path.join(process.cwd(), '.env'),
    }),
    ThrottlerModule.forRootAsync({
      ...
      useFactory: (config: ConfigService) => ({
        ...
        throttlers: [
          ...
          {
            name: 'interview',
            ttl: Number(config.get('RATE_LIMIT_INTERVIEW_TTL_MS', 60_000)),
            limit: Number(config.get('RATE_LIMIT_INTERVIEW_LIMIT', 30)),
            getTracker: (req: Record<string, unknown>) => {
              const u = req.user as { id?: string } | undefined;
              const id = u?.id;
              return id
                ? `interview:user:${id}`
                : `interview:ip:${(req.ip as string) || 'unknown'}`;
            },
          },
        ],
      }),
