...
export class WalletService {
  ...
  private buildClient(): { client: Client; operatorId: AccountId; operatorKey: PrivateKey } {
  ...
  private get arenaTokenId(): TokenId {
    ...
    if (!id) throw new InternalServerErrorException('ARENA_COIN_TOKEN_ID is not configured in .env');
  ...
  async adminMintToCompany(userId: string, amount: number) {
    ...
    const user = await this.prisma.user.findUnique({
      where: { id: userId },
      select: { id: true, firstName: true, lastName: true, role: true, hederaAccountId: true, walletBalance: true },
    });
    ...
    if (!tokenIdStr) {
      throw new InternalServerErrorException('ARENA_COIN_TOKEN_ID is not configured in .env');
    }
    ...
    try {
      ...
      this.logger.log(`Transferred ${amount} ARENA → ${user.hederaAccountId} (${receipt.status})`);
    } catch (err: any) {
      client.close();
  ...
  async lockEscrow(
    ...
    const tokenIdStr = this.config.get<string>('ARENA_COIN_TOKEN_ID') ?? 'NOT_SET';
  ...
  async releaseRewardToWinner(
    ...
    const winner = await this.prisma.user.findUnique({
      ...
      select: { id: true, firstName: true, lastName: true, hederaAccountId: true, walletBalance: true },
    });
    ...
    if (winner.hederaAccountId && tokenIdStr !== 'NOT_SET') {
      ...
      if (!winner.hederaAccountId) {
        txStatus = TransactionStatus.PENDING_ASSOCIATION;
        errorNote = 'Winner has no Hedera wallet registered. Use PATCH /user/wallet to add one, then claim the reward.';
      } else {
        errorNote = 'ARENA_COIN_TOKEN_ID not configured — reward logged off-chain only';
      }
    ...
    if (txStatus === TransactionStatus.SUCCESS) {
      await this.prisma.user.update({
        where: { id: winnerUserId },
        data: { walletBalance: { increment: amount }, totalWins: { increment: 1 } },
      });
    }
  ...
  async refundEscrow(
    ...
    if (company.hederaAccountId && tokenIdStr !== 'NOT_SET') {
      ...
      errorNote = 'Refund is off-chain only (no Hedera wallet or token not configured)';
    }
