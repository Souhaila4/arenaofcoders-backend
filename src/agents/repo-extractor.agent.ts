...
export class RepoExtractorAgent implements Agent<string, RepoMetadata> {
  ...
  async execute(repoUrl: string): Promise<RepoMetadata> {
    ...
    const readme = await this.githubApi.requestText(
      `/repos/${parsed.owner}/${parsed.repo}/readme?ref=${targetBranch}`,
    );

    const treeResponse = await this.githubApi.request<{
      tree: Array<{ path: string; type: string; size?: number }>;
    }>(
      `/repos/${parsed.owner}/${parsed.repo}/git/trees/${encodeURIComponent(targetBranch)}?recursive=1`,
    );

    const languagesResponse = await this.githubApi.request<
    ...
    return {
      ...
      repo: repo.name,
      defaultBranch: targetBranch, // Override the reported default branch so orchestrator knows what was evaluated
      readme,
      tree: paths,
      treeWithSize,
  ...
  private safeParse(url: string): { owner: string; repo: string; branch?: string } {
    try {
      return this.githubApi.parseRepoUrl(url);
    } catch {
      throw new BadRequestException(
        'githubUrl must be a valid GitHub repo URL',
      );
    }
  }
}
