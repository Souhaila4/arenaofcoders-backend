...
export class EquipeService {
  ...
  /**
   * INVITATION D'UN MEMBRE À UNE ÉQUIPE
   * 
   * Permet au Leader d'une équipe d'inviter un autre développeur (par email).
   * Vérifications effectuées :
   * 1. L'équipe n'est pas déjà pleine (MAX_TEAM_MEMBERS).
   * 2. L'invité n'est pas déjà dans une autre équipe pour ce même hackathon.
   * 3. (Exception) : Si l'invité participe en "Solo", il PEUT quand même être invité.
   * 
   * Génère ensuite une `EquipeInvitation` (Statut: PENDING) et envoie une Notification.
   */
   ...
   * 
   * L'utilisateur invité accepte de rejoindre l'équipe.
   * Le système vérifie que l'invitation lui est bien destinée et que l'équipe n'est pas pleine.
   * 
   * Dans une transaction Prisma :
   * 1. L'invitation passe en statut ACCEPTED.
   * 2. L'utilisateur est ajouté comme membre (EquipeMember).
   * 3. Toutes ses autres invitations en attente sont annulées (DECLINED).
   * 4. S'il était un participant Solo, son `CompetitionParticipant` est rattaché à cette équipe.
   * 5. Si l'équipe atteint la taille maximale, elle passe automatiquement en statut `READY`.
   */
  ...
  async searchUsers(query?: string, competitionId?: string, specialty?: string) {
    const where: any = {
      role: UserRole.USER,
      isBanned: false,
    };
    ...
    if (specialty) {
      where.mainSpecialty = specialty.toUpperCase() as Specialty;
    }
