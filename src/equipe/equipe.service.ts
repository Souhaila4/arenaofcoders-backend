...
export class EquipeService {
  ...
  async inviteToEquipe(equipeId: string, dto: InviteToEquipeDto, inviterId: string) {
    ...
    if (equipe.members.length >= MAX_TEAM_SIZE) {
      throw new BadRequestException('Team is already full (3/3 members)');
    }
  ...
  async acceptInvitation(invitationId: string, userId: string) {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      // Update equipe status if now full
      const newMemberCount = equipe.members.length + 1;
      if (newMemberCount >= MAX_TEAM_SIZE) {
  ...
  async autoAssignSoloUsers(competitionId: string) {
    ...
    // Group into chunks of MAX_TEAM_SIZE
    for (let i = 0; i < shuffled.length; i += MAX_TEAM_SIZE) {
      const chunk = shuffled.slice(i, i + MAX_TEAM_SIZE);

      // Pick random leader
      const leaderIndex = Math.floor(Math.random() * chunk.length);

      const equipe = await this.prisma.$transaction(async (tx) => {
        const teamNumber = Math.floor(i / MAX_TEAM_SIZE) + 1;
        const newEquipe = await tx.equipe.create({
          data: {
            name: `Équipe Auto #${teamNumber}`,
            competitionId,
            status:
              chunk.length >= MAX_TEAM_SIZE
                ? EquipeStatus.READY
